...
export function ModuleTabContainer({
  ...
  return (
    <div className="flex flex-col w-full h-full flex-1 min-h-0 space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-300/60 dark:border-slate-800/60 pb-2.5 gap-3 shrink-0">
        <div className="flex items-center gap-3">
          {title && (
            <div className="flex items-center gap-2">
              {Icon && <Icon className="h-4 w-4 text-indigo-500 dark:text-indigo-400" />}
              <h2 className="text-xs font-extrabold text-slate-700 dark:text-slate-200 tracking-wider uppercase">
                {title}
              </h2>
            </div>
          )}
          ...
          {onMaximizeToggle && (
            <button
              ...
              className={`p-1 rounded hover:bg-slate-300/50 dark:hover:bg-[#16223f]/50 border border-transparent hover:border-slate-300 dark:hover:border-slate-800 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 cursor-pointer transition-all duration-200 focus:outline-none`}
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          ...
          <div className="flex items-center bg-slate-300/30 dark:bg-[#0e1626]/30 p-1 rounded-lg gap-1 border border-slate-300/50 dark:border-slate-850/40">
            {tabs.map((tab) => {
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer focus:outline-none ${
                    isActive
                      ...
                      : "text-slate-650 dark:text-slate-400 hover:text-slate-800 hover:dark:text-slate-200 hover:bg-slate-300/40 hover:dark:bg-[#16223f]/30"
                  }`}
                >
          ...
          {isMaximized && onMaximizeToggle && (
            <button
              onClick={onMaximizeToggle}
              className="flex items-center gap-1.5 px-2.5 py-1 bg-red-50/50 dark:bg-red-950/20 hover:bg-red-100 dark:hover:bg-red-950/40 border border-red-200 dark:border-red-900/40 hover:border-red-300 dark:hover:border-red-900/60 text-red-650 dark:text-red-400 rounded-lg text-[9px] font-bold uppercase tracking-wider cursor-pointer transition-all duration-200 focus:outline-none"
